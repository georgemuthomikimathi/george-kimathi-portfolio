    ...
    <div className="min-h-screen">
      ...
      <header className="border-b border-navy/10 bg-white">
        <div className="mx-auto flex max-w-5xl items-center justify-between px-6 py-4">
          <p className="pro-heading text-lg font-semibold text-navy">{george.shortName}</p>
          <nav className="flex gap-4 text-sm text-navy/70">
            <a href="#timeline" className="hover:text-navy">Timeline</a>
            <a href="#credentials" className="hover:text-navy">Credentials</a>
            <a href="#projects" className="hover:text-navy">Projects</a>
          </nav>
        </div>
      </header>

      <section className="border-b border-navy/10 bg-white">
        <div className="mx-auto grid max-w-5xl gap-10 px-6 py-20 md:grid-cols-[1fr_200px] md:items-center">
          <div>
            ...
            <p className="mt-4 max-w-xl text-navy/70">{george.tagline}</p>
            <div className="mt-6 flex flex-wrap gap-3">
              <a href={george.github} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-full bg-navy px-5 py-2.5 text-sm text-white hover:bg-navy-light">
                <Github className="h-4 w-4" /> GitHub
              </a>
              <a href="/dossier/George-Kimathi-Credentials-Public.pdf" download className="inline-flex items-center gap-2 rounded-full border border-navy/20 px-5 py-2.5 text-sm text-navy hover:bg-navy/5">
      ...
      <section id="timeline" className="border-y border-navy/10 bg-white py-16">
            ...
            {george.timeline.map((item, i) => (
              <div key={item.year} className="relative flex gap-6 pb-10">
                {i < george.timeline.length - 1 && (
                  <div className="absolute left-[7px] top-3 h-full w-px bg-navy/15" />
                )}
                <div className="mt-1.5 h-3.5 w-3.5 shrink-0 rounded-full border-2 border-gold bg-white" />
                <div>
                  <p className="text-xs font-semibold uppercase tracking-wider text-gold">{item.year}</p>
                  <p className="mt-1 font-semibold text-navy">{item.place} <span className="font-normal text-navy/50">{item.country}</span></p>
      ...
      <section id="credentials" className="mx-auto max-w-5xl px-6 py-16">
        ...
        <p className="mt-2 text-navy/60">Verified academic records & merit awards</p>
        <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {george.credentials.map((cred) => (
            <button
              ...
              className="group overflow-hidden rounded-xl border border-navy/10 bg-white text-left shadow-sm transition hover:shadow-md"
            >
      ...
      <section id="projects" className="mx-auto max-w-5xl px-6 py-16">
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-2">
          {george.projects.map((project) => (
            <a
              key={project.name}
              href={project.url === "/shop-link" ? "http://localhost:3003" : project.url}
              target="_blank"
              rel="noreferrer"
              className="flex items-start justify-between rounded-xl border border-navy/10 bg-white p-5 shadow-sm transition hover:border-gold/40 hover:shadow-md"
            >
              <div>
                <p className="font-semibold text-navy">{project.name}</p>
                <p className="mt-1 text-sm text-navy/60">{project.desc}</p>
              </div>
      ...
      <footer className="border-t border-navy/10 bg-navy py-10 text-center text-sm text-white/70">
        ...
        <p className="mt-2">
          <a href="http://localhost:3002" className="underline hover:text-white">Vibe site</a>
          {" · "}
          <a href="http://localhost:3003" className="underline hover:text-white">GK Lab store</a>
        </p>
      </footer>
    </div>
  );
